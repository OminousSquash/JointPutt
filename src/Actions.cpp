...
void Actions::ReleaseBall(const std::optional<sf::Event> &event, Ball &b1, Ball& b2, sf::Vector2f& mousePos) {
    if (const auto* mouseButtonRelease= event->getIf<sf::Event::MouseButtonReleased>()) {
        if (this->ball1Held)  {
            ...
            int mouseY = mouseButtonRelease -> position.y;
            b2.xVel = b2.x - mouseX;
            b2.xVel = b2.xVel > CONSTANTS::MAX_X_VELOCITY ? CONSTANTS::MAX_X_VELOCITY : b2.xVel;
            b2.xVel = b2.xVel < CONSTANTS::MIN_X_VELOCITY ? CONSTANTS::MIN_X_VELOCITY: b2.xVel;

            b2.yVel = b2.y - mouseY;
            b2.yVel = b2.yVel > CONSTANTS::MAX_Y_VELOCITY ? CONSTANTS::MAX_Y_VELOCITY : b2.yVel;
            b2.yVel = b2.yVel < CONSTANTS::MIN_Y_VELOCITY ? CONSTANTS::MIN_Y_VELOCITY : b2.yVel;
            setVelocities(b1, b2.xVel, b2.yVel);
        }
    }
}
